...
IRModule IOAllocateCreator::operator()() {
  ...
  mod_->Update(
      gv, PrimFunc(new_main_params, main_body, main_func_->ret_type, main_func_->buffer_map,
                   main_func_->preflattened_buffer_map, main_func_->sp_axes,
                   main_func_->attrs, main_func_->span));
  return mod_;
}
