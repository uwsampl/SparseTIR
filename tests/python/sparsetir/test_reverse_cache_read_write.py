...
def tcspmm_rev_cache_read(
    ...
    for io in T.serial(128):
        with T.block("tcspmm0"):
            ...
            T.reads(
                JO_indptr[vio : vio + 2],
                A[vio, 0:128, 0:16, 0:16],
                B[0:2048, 0:64],
                JI_indices[vio, 0:128, 0:16],
            )
            ...
            for jo, f_0 in T.grid(JO_indptr[vio + 1] - JO_indptr[vio], 4):
                with T.block("tcspmm1_o"):
                    ...
                    T.reads(
                        A[vio, vjo, 0:16, 0:16],
                        B[0:2048, vf_o * 16 : vf_o * 16 + 16],
                        JI_indices[vio, vjo, 0:16],
                    )
                    ...
                    B_shared_wmma_matrix_b = T.alloc_buffer(
                        [16, 16], dtype="float16", scope="wmma.matrix_b"
                    )
                    with T.init():
                    ...
                    for ii, ji, f_1 in T.grid(16, 16, 16):
                        with T.block("tcspmm1"):
                            ...
                            T.reads(
                                C[vio, vii, vf_o * 16 + vf],
                                A[vio, vjo, vii, vji],
                                B_shared_wmma_matrix_b[vji, vf],
                                JI_indices[vio, vjo, vji],
                            )
                            T.writes(C[vio, vii, vf_o * 16 + vf])
                            T.block_attr({"sparse": True})
                            C[vio, vii, vf_o * 16 + vf] = (
                                C[vio, vii, vf_o * 16 + vf]
                                + A[vio, vjo, vii, vji] * B_shared_wmma_matrix_b[vji, vf]
                            )


@T.prim_func
def tcspmm_rev_cache_read_21(
    a: T.handle, b: T.handle, c: T.handle, indptr: T.handle, indices: T.handle
) -> None:
    # function attr dict
    T.func_attr({"global_symbol": "main", "tir.noalias": True, "sparse_tir_level": 1})
        ...
        with T.block("tcspmm0"):
            ...
            for jo, f_0 in T.grid(JO_indptr[vio + 1] - JO_indptr[vio], 4):
                with T.block("tcspmm1_o"):
                    ...
                    for ax0, ax1 in T.grid(16, 16):
                        with T.block("B_shared"):
                            v0, v1 = T.axis.remap("SS", [ax1, ax0])
                            T.reads(B[JI_indices[vio, vjo, v1], vf_o * 16 + v0])
                            T.writes(B_shared[v0, v1])
                            T.block_attr({"sparse": True})
                            B_shared[v0, v1] = B[JI_indices[vio, vjo, v1], vf_o * 16 + v0]
                    for ii, ji, f_1 in T.grid(16, 16, 16):
                        with T.block("tcspmm1"):
                            vii, vji, vf = T.axis.remap("SRS", [ii, ji, f_1])
                            T.reads(
                            ...
                            C[vio, vii, vf_o * 16 + vf] = (
                                C[vio, vii, vf_o * 16 + vf]
                                + A[vio, vjo, vii, vji] * B_shared[vf, vji]
                            )
...
@T.prim_func
def scatter_add_rev_cache_write(
    A: T.Buffer[(128, 16), "float32"], B: T.Buffer[(1024,), "float32"], I: T.Buffer[(128,), "int32"]
) -> None:
    # body
    # with T.block("root")
...
def test_tc_spmm_cache_read_dim_order():
    MB, NB, NNZB, F, B = tcspmm.params[-5:]
    mod = tvm.IRModule.from_expr(tcspmm.specialize({MB: 128, NB: 128, NNZB: 1024, F: 64, B: 16}))
    mod = lower_sparse_iter(mod)
    sch = tir.Schedule(mod)
    ...
    sch.reorder(fo, ii, ji, fi)
    new_blk = sch.blockize(ii)
    B_shared = sch.reverse_cache_read(blk_inner, 2, "shared", [2, 1])
    tvm.ir.assert_structural_equal(sch.mod["main"], tcspmm_rev_cache_read_21, True)
...
if __name__ == "__main__":
    ...
    test_tc_spmm_cache_read_dim_order()
